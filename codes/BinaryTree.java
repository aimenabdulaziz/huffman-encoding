...
//		return Math.max(left_height, right_height) + 1;
		...
		return h+1;						// inner: one higher than the highest child
	...
	public BinaryTree<E> copyToDepth(int d) {
		BinaryTree<E> leftCopy;
		BinaryTree<E> rightCopy;
		if (d == 0){
			return new BinaryTree<E>(this.data);
		}

		else if (hasLeft() && hasRight()){
			leftCopy = left.copyToDepth(d-1);
			rightCopy = right.copyToDepth(d-1);
			return new BinaryTree<E>(data, leftCopy, rightCopy);
		}

		else if (hasLeft()){
			leftCopy = left.copyToDepth(d-1);
			return new BinaryTree<E>(data, leftCopy, null);
		}

		else if (hasRight()){
			rightCopy = right.copyToDepth(d-1);
			return new BinaryTree<E>(data, null, rightCopy);
		}
		return new BinaryTree<E>(this.data);
		/**
		 * option II
